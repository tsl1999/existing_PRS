...
#data readin-----------------------------------------------------------------
data<-readRDS(paste(data_path,"/RestrictedData_22Jun2023.rds",sep=""))
model_table_top<-readRDS(paste(data_path,"/model_table_top.rds",sep=""))

data_cont<-data[,1:39]
data_cat<-data[,c(1:32,40:46)]
...
saveRDS(model_withoutPRS_EPA,paste(data_path,"/Prevalent_CHD_EPA_withoutPRS_restricted3rd.rds",sep=""))
...
saveRDS(flex_table,paste(data_path,"/Prevalent_CHD_EPA_cont_restricted3rd_flex.rds",sep=""))
...
png(paste(graphs_path,"/logistic/Logistic_forestplotEPA_cont_restricted3rd",".png",sep=""), res = 200, width = a[1], height = a[2], units = "cm")
print(p)
dev.off()
...
plot_FAR_gg(model_output_list,name=c("Partial","Full"),
            outcome="Prevalent_CHD_EPA_restricted3rd",data=data)
...
saveRDS(model_withoutPRS_EPO,paste(data_path,"/Prevalent_CHD_EPO_withoutPRS_restricted3rd.rds",sep=""))
...
saveRDS(flex_table,paste(data_path,"/Prevalent_CHD_EPO_cont_restricted3rd_flex.rds",sep=""))
...
png(paste(graphs_path,"/logistic/Logistic_forestplotEPO_cont_restricted3rd",".png",sep=""), res = 200, width = a[1], height = a[2], units = "cm")
print(p)
dev.off()
...
plot_FAR_gg(model_output_list,name=c("Partial","Full"),
            outcome="Prevalent_CHD_EPO_restricted3rd",data=data)
