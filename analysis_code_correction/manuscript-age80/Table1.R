...
variables_to_select<-c("AGE","COYOACAN","amrscore","afrscore","easscore","eurscore","EDU_LEVEL",
...
var_name<-c("Age,years","District (Coyoacan)","Indigenous American",
            "African","East Asian","European","Education level",
            ...
            "Cardiovascular Disease","Cancer","Diabetes (HBA1C>6.5%, self-report or medication use)",
...
baseline_up_no_missing<-rbind(baseline_hidemissing[c(1:2),],
                              c("Ancestry admixture proportion, Mean(SD)",
                                rep(NA,3)),baseline_hidemissing[c(3:28),],
                              c("Physiological measures, Mean(SD)",rep(NA,3)),
                              baseline_hidemissing[c(29:35),],
                              c("Reoprted medical history, N(%)",rep(NA,3)),
                              baseline_hidemissing[c(36:40),])

rownames(baseline_up_no_missing)<-seq(1:nrow(baseline_up_no_missing))

for (i in c(4:7,31:37,39:43)){
  baseline_up_no_missing[i,1]<-paste('\U{00A0}\U{00A0}',gsub(",.*$", "",baseline_up_no_missing[i,1]),sep="")
}

baseline_up_no_missing[42,1]<-"\U{00A0}\U{00A0}Diabetes (HBA1C>6.5%, self-report or medication use)"

## flextable---------------------------------------------------------
flex_baseline<-my_table(baseline_up_no_missing, y=c(1:3,8,13,18,24,30,38))

flex_baseline <- add_header_row(flex_baseline, values =  c("Characteristic", "Sex"),
...
flex_baseline<-footnote(flex_baseline,i=43,j=1,part="body",ref_symbols="§",
...
flex_baseline<-set_caption(
  flex_baseline,
  caption = "Baseline Characteristics of men and women aged 35-80 years",align_with_table = FALSE,
  word_stylename = "Table Caption",
  fp_p = fpp
)

flex_baseline<-my_theme(flex_baseline,y=c(1:43))
saveRDS(flex_baseline,paste(data_path,"/manuscript-age80/table1_nomissing_flex_80.rds",sep=""))
flex_baseline
