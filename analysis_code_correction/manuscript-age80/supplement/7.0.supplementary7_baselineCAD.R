...
model_output_partial<-create_output_table(
  trainsplit = F,data,adjustments=partial_adjustments,outcome="BASE_CHD",namew="partial",roc=F)

model_output_full<-create_output_table(
  trainsplit = F,data,adjustments=full_adjustments,outcome="BASE_CHD",namew="full",roc=F)


write.csv(model_output_partial,paste(data_save_path,"/baseline_partial.csv",sep=""))
saveRDS(model_output_partial,paste(data_save_path,"/baseline_partial.rds",sep=""))
write.csv(model_output_full,paste(data_save_path,"/baseline_full.csv",sep=""))
saveRDS(model_output_full,paste(data_save_path,"/baseline_full.rds",sep=""))

##forest table-------------------------------------------------------------

forest_table_base<-generate_forest_table(
  tables=list(model_output_partial,model_output_full),
  model_name = c("Partially adjusted","Fully adjusted"),or_hr = "OR",show_pgsname = T)

p<-plot_forest(forest_table = forest_table_base,xlim=c(0.9,1.4),ticks_at = c(0.9,1,1.1,1.2,1.3),hr_or="OR",
               footnote_in=" ")
a<-get_wh(p,unit = "cm")+2

png(paste(graphs_path,"/baselineCAD_forest.png",sep=""), res = 200, width = a[1], height = a[2], units = "cm")
print(p)
dev.off()
...
model_output_partial<-create_output_table(
  ...
  outcome="BASE_CHD",namew="partial",type = "cat",roc = F)

model_output_full<-create_output_table(
  trainsplit = F,data_up,adjustments=full_adjustments,
  outcome="BASE_CHD",namew="full",type = "cat",roc = F)


## calculate floating absolute risk and plot -----------
model_output_list<-list(model_output_partial,model_output_full)

plot_FAR_gg(model_output_list,name=c("Partial","Full"),outcome="BaselineCAD",data = data,
