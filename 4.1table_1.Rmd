...
colnames(data)[35:41]<-sub("_standardised","",colnames(data)[35:41])#remove standardised after all PRS names to make column names shorter
colnames(data)[41]<-"custom_Oni_Orisan"# change custom_Oni-Orisan to Oni_Orisan so that formula() can read it
...
```{r,include=FALSE}
prs_cor<-cor(data[,35:41],method = "pearson")
...
table(data$prevalent_CHD_EPA,exclude = NULL)#5488 prevalent cases
table(data$prevalent_CHD_EPO,exclude = NULL)#4921
...
baseline_up[42,1]<-"\U{00A0}\U{00A0}Diabetes (HBA1C>6.5%, self-report or medication use)"



#saveRDS(baseline_up,paste(data_path,"/table1_31May2023.rds",sep=""))
```
...
flex_baseline<-footnote(flex_baseline,i=42,j=1,part="body",ref_symbols="§",value = as_paragraph("Other diseases include self-reported emphysema, chronic kidney disease, peptic ulcer,cirrhosis, and peripheral arterial disease."))
...
flex_baseline<-my_theme(flex_baseline,y=c(1:42))
saveRDS(flex_baseline,paste(data_path,"/table1_flex.rds",sep=""))
flex_baseline
```
...
flex_baseline<-footnote(flex_baseline,i=32,j=1,part="body",ref_symbols="§",value = as_paragraph("Other diseases include self-reported emphysema, chronic kidney disease, peptic ulcer,cirrhosis, and peripheral arterial disease."))
...
flex_baseline<-my_theme(flex_baseline,y=c(1:32))
saveRDS(flex_baseline,paste(data_path,"/table1_nomissing_flex.rds",sep=""))
flex_baseline
```
...
for (i in 1:7){
  ...
  quintile<-cut(data[,34+i], breaks=quintile_cutoff,labels = c(1:5),include.lowest=TRUE)
...
list_quintile<-NA
for (i in 1:7){
  pgs_quintile<-create_table_1(data=data_prs,variables_to_select = variables_to_select,var_name = var_name,by=paste(pgsname[i],"_quintile",sep=""),compare = T,self_define=5,missing = F)
  colnames(pgs_quintile)[1]<-gsub("_standardised","",pgsname[i])
  pgs_quintile<-pgs_quintile[,c(1:6,8)]
  list_quintile[i]<-list(pgs_quintile)
}

#saveRDS(list_quintile,paste(data_path,"/PRS_quintile_tables_31May2023.rds",sep=""))
```
...
for (i in 1:7){
  ...
  flex_pgs<-my_table(list_quintile[[i]], y=c(2:10,12,15,20,26,31:35),x=1:7)
...
#flex_pgs<-footnote(flex_pgs,i=45,j=1,part="body",ref_symbols="§",value = as_paragraph("Other diseases include self-reported emphysema, chronic kidney disease, peptic ulcer,cirrhosis, and peripheral arterial disease."))
flex_pgs<-footnote(flex_pgs,i=35,j=1,part="body",ref_symbols="§",value = as_paragraph("Other diseases include self-reported emphysema, chronic kidney disease, peptic ulcer,cirrhosis, and peripheral arterial disease."))
...
flex_pgs<-set_caption(
 flex_pgs,
  caption = paste(sub("_standardised","",pgsname[i]), "quintile table"),align_with_table = FALSE,
 word_stylename = "Table Caption",
  fp_p = fpp
)
...
p[i]<-list(my_theme(flex_pgs,y=c(1:34),z=1:6,fontsize = 6.8))
}

saveRDS(p,paste(data_path,"/PRS_quintile_tables_nomissing_flex.rds",sep=""))
