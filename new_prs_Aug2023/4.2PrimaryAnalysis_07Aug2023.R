...
colnames(data)[33:ncol(data)]<-sub("_standardised","",colnames(data)[33:ncol(data)])
...
variants_included<-readRDS(paste(data_path,"variants_included_07Aug2023.rds",sep="/"))
...
variant_t[2:length(variant_t)]<-prettyNum(variant_t[2:length(variant_t)], big.mark = ",", scientific = FALSE)
model_table_top<-rbind(variant_t,PGS_id_name[c(2,1),])
model_table_top[2,1]<-"Publication date"
m_t<-t(model_table_top)
colnames(m_t)<-m_t[1,]
m_t<-m_t[-1,]
m_t<-m_t[c(order(m_t[1:4,2]),order(m_t[5:8,2])+4),]
...
saveRDS(model_table_top,paste(data_path,"/model_table_top_07Aug2023.rds",sep=""))
...
saveRDS(flex_table,paste(data_path,"/Prevalent_CHD_EPA_cont_logistic_flex_07Aug2023.rds",sep=""))
...
png(paste(graphs_path,"/logistic/Logistic_forestplotEPA_cont_07Aug2023",".png",sep=""), res = 200, width = a[1], height = a[2], units = "cm")
print(p)
dev.off()
...
saveRDS(flex_table,paste(data_path,"/Prevalent_CHD_EPO_cont_logistic_flex_07Aug2023.rds",sep=""))
...
p<-plot_forest(forest_table = forest_table_EPO,xlim=c(0.9,1.4),ticks_at = c(0.9,1,1.1,1.2,1.3),hr_or="HR")
a<-get_wh(p,unit = "cm")+2

png(paste(graphs_path,"/logistic/Logistic_forestplotEPO_cont_07Aug2023",".png",sep=""), res = 200, width = a[1], height = a[2], units = "cm")
print(p)
dev.off()
